...
PYBIND11_PLUGIN(nmsbind) {
    ...
    m.def("init",
        [](const std::string & space, py::object space_params, const std::string & method,
            ...
            switch (dtype) {
                case DISTTYPE_FLOAT: {
                    auto index = new IndexWrapper<float>(method, space, space_params,
                                                         data_type, dtype);
                    ret = py::cast(index);
                    break;
                }
                case DISTTYPE_DOUBLE: {
                    auto index = new IndexWrapper<double>(method, space, space_params,
                                                          data_type, dtype);
                    ret = py::cast(index);
                    break;
                }
                case DISTTYPE_INT: {
                    auto index = new IndexWrapper<int>(method, space, space_params,
                                                       data_type, dtype);
                    ret = py::cast(index);
                    break;
                }
        ...
        "dist_type: nmsbind.DistType optional\n"
